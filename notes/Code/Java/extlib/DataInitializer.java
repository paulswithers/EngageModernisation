...
public class DataInitializer {
	...
	void createUsers(Database db) throws NotesException, IOException {
		...
		Object tmpReaders = ExtLibUtil.getViewScope().get("readers");
		if (tmpReaders instanceof Vector) {
			readers.addAll((Vector) ExtLibUtil.getViewScope().get("readers"));
		}
		...
		for (int i = 0; i < users_maxUsers; i++) {
			while (true) {
				...
				if (users == null || !users.contains(nn)) {
					...
					createUser(db, id, firstName, lastName, city, state, email, readers);
					break;
				}
			}
		}
	...
	void createUser(Database db, String id, String firstName, String lastName, String city, String state, String email,
			Vector<String> readers) throws NotesException {
		...
		try {
			...
			Item configReaders = doc.replaceItemValue("configReaders", "[Configuration]");
			configReaders.setReaders(true);
			if (!readers.isEmpty()) {
				int count = (int) (Math.random() * readers.size());
				Vector<String> selReaders = new Vector<String>();
				if (count == readers.size()) {
					selReaders.addAll(readers);
				} else {
					while (count >= 0) {
						int idex = (int) (Math.random() * readers.size());
						if (!selReaders.contains(readers.get(idex))) {
							selReaders.add(readers.get(idex));
							count = count - 1;
						}
					}
				}
				Item userReaders = doc.replaceItemValue("Readers", selReaders);
				userReaders.setReaders(true);
			} else {
				Item userReaders = doc.replaceItemValue("Readers", "*");
				userReaders.setReaders(true);
			}
			doc.save();
		} finally {
	...
	void createDiscussionDocuments(Database db) throws NotesException, IOException {
		...
		try {
			...
			for (ViewEntry e = authorCol.getFirstEntry(); e != null
					&& nAuthor < maxAuthors; e = authorCol.getNextEntry()) {
